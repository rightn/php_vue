            ...
            <tr v-for="(item, index) in todos" :key="index">
              ...
              <td>
                <div class="btn-group" role="group">
                  <button class="btn btn-danger" @click="editTodo(item.id)">수정</button>
...
export default {
  ...
  setup() {
    ...
    // 서버에서 자료를 읽어오기
    const getInfo = () => {
      fetch('http://rightn.dothome.co.kr/data_read.php')
      .then(res => res.json())
      .then(data => {        
    ...
    const deleteTodo = (_id) => {
      // console.log(_id);
      fetch(`http://rightn.dothome.co.kr/data_delete.php?id=${_id}`)
      .then(res => res.json())
      .then(data => {
        // console.log(data);
        // 목록갱신
